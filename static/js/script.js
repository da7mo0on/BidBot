document.addEventListener('DOMContentLoaded', () => {
    ...
    let selectedLines = [];
    let selectedLinesOrder = [];
    let currentDuties = null;
    let currentLineDuties = null;

    document.querySelector('.control-btn:first-child').addEventListener('click', () => {
        $('#filtersModal').modal('show');
    });

    // Handle file upload with real progress
    uploadForm.addEventListener('submit', (e) => {
        ...
        // إعداد SSE لاستقبال تحديثات التقدم
        const source = new EventSource('/progress');

        // مهلة زمنية (30 ثانية) لـ SSE
        const timeout = setTimeout(() => {
            console.error('SSE connection timed out after 30 seconds');
            source.close();
            progressStatus.textContent = 'Error: Connection timed out. Please try again.';
        ...
        source.onmessage = (event) => {
            ...
            if (percentage >= 100 && data.redirect) {
                clearTimeout(timeout);
                source.close();
                // إعادة توجيه مباشرة إلى النتائج
                setTimeout(() => {
        ...
        source.onerror = () => {
            ...
            progressStatus.textContent = 'Error: Failed to connect to server. Please try again.';
            progressBar.style.display = 'none';
        };
    ...
    function handleSortClick() {
        ...
        if (newOrder === 'default') {
            sortedRows = currentFilteredRows;
        ...
        const filteredLines = lines.filter(line => {
            ...
            const layoverDurations = pairings.flatMap(p =>
                ...
                p.details.split('\n').filter(l => l.includes('LAYOVER')).map(l => {
                    ...
                    if (match) {
                        const city = match[1];
                        if (localAirports.includes(city)) return null;
            ...
            if (pairingsCount < pairingsMin || pairingsMax < pairingsCount) return false;
    ...
    function handleCheckboxChange() {
        ...
        if (this.checked) {
            ...
            selectedLines = selectedLines.filter(num => num !== lineNumber);
        }
        updateBidSheetButton();
    }
        ...
        checkboxes.forEach(checkbox => {
            checkbox.addEventListener('change', () => {
                ...
                if (!checkbox.checked) {
                    row.remove();
                    selectedLines = selectedLines.filter(num => num !== lineNumber);
                    selectedLinesOrder = selectedLinesOrder.filter(num => num !== lineNumber);
    ...
    function updateCopyLines() {
        const lineNumbers = Array.from(document.getElementById('bid-lines-table-body').querySelectorAll('tr'))
            .filter(row => !row.classList.contains('details-row'))
        ...
        selectedLinesOrder = lineNumbers;
    }
